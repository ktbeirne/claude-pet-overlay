...
- フレーム: 透過 PNG、セル比 192:208 (推奨 576x624)。再生タイミングは
  `timing.yaml` (`fps: 8` または `durations_ms: [5000, 250]`) で宣言 (既定 60fps)。
  旧形式の `fps.txt` / `durations.txt` も可だが、.txt を自動暗号化する企業環境で
  読めなくなる実害があったため yaml を推奨。優先順: timing.yaml > durations.txt > fps.txt
