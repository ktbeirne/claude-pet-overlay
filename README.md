...
初回はプレースホルダ素材 (`Assets\Frames.placeholder`) が `Assets\Frames` へコピーされ、
Release ビルド後に起動します。キャラ素材を使う場合は `Assets\Frames\<state>\frame_*.png`
(+ `fps.txt` / `durations.txt`) を差し替えてください。
...
`-FramesDir` を指定しない場合はプレースホルダ素材が同梱されます。
